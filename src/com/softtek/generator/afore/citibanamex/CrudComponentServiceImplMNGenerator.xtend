...
class CrudComponentServiceImplMNGenerator {
	
	def doGenerate(Resource resource, IFileSystemAccess2 fsa) {
		for (m : resource.allContents.toIterable.filter(typeof(Module))) {
			for (e : m.elements.filter(typeof(Entity))) {
				fsa.generateFile("banamex/mn/src/main/java/com/aforebanamex/plata/cg/mn/service/impl/" + e.name.toLowerCase.toFirstUpper + "ServiceImpl.java", e.genServiceMNServiceImpl(m))
			}
		}
	}
	
	def CharSequence genServiceMNServiceImpl(Entity e, Module m) '''
