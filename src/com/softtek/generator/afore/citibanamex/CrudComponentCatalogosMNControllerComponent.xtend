...
class CrudComponentCatalogosMNControllerComponent {
	
	def doGenerate(com.softtek.rdl2.System s, IFileSystemAccess2 fsa) {
		fsa.generateFile("banamex/mn/src/main/java/com/aforebanamex/plata/cg/mn/controller/CatalogosMNController.java", genCatalogoMNController(s, fsa))	
	}
	
	def CharSequence genCatalogoMNController(com.softtek.rdl2.System s, IFileSystemAccess2 fsa) '''
	package com.aforebanamex.plata.cg.mn.controller;
	
	import java.util.List;
	
	import com.aforebanamex.plata.cg.mn.service.CatalogosMNService;
	...
	@Controller
	public class CatalogosMNController {
	...
	def dispatch getEntityFieldRel(Enum e, String name) '''
		@RequestMapping(value="/obtenerCatalogo«e.name.toLowerCase.toFirstUpper»")
		public @ResponseBody List<«e.name.toLowerCase.toFirstUpper»> obtenerCatalogo«e.name.toLowerCase.toFirstUpper»(){
			return catalogosMNService.obtenerCatalogo«e.name.toLowerCase.toFirstUpper»();
		}
	'''
